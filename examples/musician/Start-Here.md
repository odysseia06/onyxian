...
- **music** 0.2.0 — A complete musician's workspace: theory, practice (logs, routines, technique), composition (ideas, melodies, lyrics, song structures), production, listening notes, a knowledge shelf, and copy-per-piece music projects — eleven templates and a practice-log Base.
...
- **music**: Fill Goals.md first; log every session with the Practice Log template (the Base tracks them); the practice-coach agent reads priorities and targets from Goals.md and only from there, and will ask rather than invent them while it is empty. Keep loose ideas in Composition; a piece earns a folder under Projects (copy _Project-Template) only when it gets serious.
