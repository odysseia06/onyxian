...
Tend the two shelves in General. Prompts: one note per prompt in General/Prompts, named by what it is for; prompts are living notes, edited in place — an improvement replaces the old text, never a dated copy. Drafts: one note per skill draft in General/Agent-Skills with a truthful `status`; the lifecycle is draft → testing → graduated and moves only when the user says so — record it, never advance it yourself. Graduation records where it shipped (the repo, marketplace, or vault path that now owns the skill) in the draft note. Propose retiring graduated drafts into General/Agent-Skills/Archive, and answer "what's in testing" from the notes' status keys alone. The dashboard at General/00 Dashboard.md is the user's own overview note: read it, never write it.
...
- a graduated draft is ready to retire — propose the move into General/Agent-Skills/Archive, apply only after confirmation
