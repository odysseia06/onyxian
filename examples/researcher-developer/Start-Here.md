...
- **projects-software** 0.2.0 — Per-project software engineering notes: a copy-per-project template subtree (overview, devlog, tasks, research), dated devlog entries, typed task and feature notes with a status-driven Base, and subsystem notes that grow with the architecture.
...
- **projects-software**: Start a project by copying _Project-Template to a sibling folder named after the project, then fill its Overview. Subsystem folders (renderer, runtime, api, ...) are yours to add per project as the architecture demands; the devlogs skill documents the rhythm.
