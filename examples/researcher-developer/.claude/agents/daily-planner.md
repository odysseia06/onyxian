...
0. Probe the CLI before anything else, skills included: run `obsidian vault info=name` (if `obsidian` is not on PATH, find the redirector per your operating rules). If it returns nothing, Obsidian is not running or the CLI is not enabled — report that and stop. Every step below needs the live CLI, so establish it first rather than discovering the block after the skills are loaded.
