...
- `.claude/skills/` — instruction packages the agents lean on: the vault's conventions (`vault-conventions`), the safe-operations contract (`vault-operations`), and one skill per workflow (`exam-prep`, `paper-pipeline`, `fitness-review`, `reading-triage`, `daily-notes`, `task-capture`, `devlogs`, `oss-tracking`, `editorial-pipeline`, `practice-loop`, `game-wiki`, `prompt-library`). You never invoke these by name; the agents read them.
- `.claude/agents/` — one scoped agent per domain module. `daily-planner` (say "plan my day", "close out the day"), `study-coach` ("build a study plan from this syllabus"), `research-librarian` (files a typed paper summary from a PDF), `reading-triager`, `fitness-coach`, `project-steward` ("we decided X because Y" becomes a decision-log entry), `oss-scout`, `blog-editor` ("capture this post idea"), `practice-coach` ("log today's practice session"), `game-steward` ("capture this game idea"), `skill-smith` ("save this prompt"). Task capture works from a plain sentence: "add a task to fix this by Friday".
...
**A recovery net: vault checkpoints.** Because scope is advisory, Onyxian offers an opt-in safety net that makes any stray write cheap to see and undo. Run `onyxian checkpoint` to snapshot the whole vault into a private git history kept under `.vault/checkpoints/` — a *separate* git repository, so it never touches a `.git` of your own, and needs no git repo to exist. `onyxian checkpoint list` shows past snapshots and `onyxian checkpoint diff` shows what changed since the last one.

Restore the common case — one mangled note — by naming it after the checkpoint id:

```
onyxian checkpoint restore c4f21a9 "Fitness/Nutrition/Strategy.md" --dry-run
onyxian checkpoint restore c4f21a9 "Fitness/Nutrition/Strategy.md"
```

You can name several paths, or omit paths to restore the whole vault. Restore always prints the exact `A` / `M` / `D` file review first and asks for confirmation (`--yes` is the non-interactive form); `--dry-run` stops after that review. It never touches the private checkpoint repository, a vault's own `.git`, `.vault/apply.lock`, half-written `*.onyxian-tmp` files, or Obsidian's volatile workspace state. Untracked files hidden by the vault's `.gitignore` are outside checkpoint scope; `.vault/config.yaml` and `.vault/lock.json` are retained anyway so recovery and ledger verification remain available. For a path-scoped restore, the matching historical ledger rows are restored too: content that was genuinely framework-owned at the checkpoint is re-verified, while a managed file you had already customized stays recognized as customized rather than being falsely claimed.

Enable checkpoints during setup (or set `framework.checkpoints: true` in `.vault/config.yaml`) and, with Claude Code, Onyxian wires a `SessionStart` hook so a checkpoint is taken automatically whenever a session starts — nothing to remember. It's a net, not a sandbox. If `git` cannot snapshot or inspect — not installed, refusing the folder, or hanging — the guard prints one warning line and moves on, so a session is never broken. Restore is different: a requested recovery that cannot run exits with an error and leaves the vault untouched, rather than pretending it succeeded. Because a hook warning is easy to miss, `onyxian doctor` also reports the guard: how many snapshots exist and when the newest was taken, or — if the guard has been running without ever managing to record one — that it is failing, and what to run to see why.

**Optional scope hooks (Claude Code).** You can turn a chunk of that convention into machinery. Enable it during setup (or set `framework.scope_hooks: true` in `.vault/config.yaml`) and, under Claude Code, Onyxian installs a `PreToolUse` hook on each agent plus a `.claude/onyxian-scopes.json` listing every agent's write globs. Before a write runs — an `obsidian` CLI command or a direct file tool (Write/Edit) — the hook checks its target and answers three ways: a write it can **prove** lands inside the agent's scope passes through untouched; one it can **prove** lands outside is **denied**; anything it **can't** prove is turned into an **ask** — the escalation you'd want anyway, made automatic. It only ever tightens: a read-only or non-`obsidian` command is never auto-approved by the hook. And because a direct file tool names its exact target path, hooked agents get Write/Edit back (only NotebookEdit stays denied): note bodies no longer have to be chunked through the CLI's `append`, and every such write is path-checked rather than trusted. Without scope hooks, agents keep the stricter tool deny-list and the CLI remains their only write path.
