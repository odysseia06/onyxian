...
Honesty notes: `writing`, `projects-gamedev`, and `ai-workspace` currently ship structure only — folders, seeds, and templates (`ai-workspace` is just folders and a dashboard note), plus a Base view for writing — but **no skills or agents yet**. `oss` deliberately defaults its root to the same folder as `projects-software`, so OSS tracking nests inside your software-projects area; change either variable if you want them apart.
