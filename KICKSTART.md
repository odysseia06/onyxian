...
- **No telemetry.** No analytics, no phone-home, no network calls beyond sources the user's config explicitly declares.
...
├─ adapters/
│  ├─ claude-code/  codex/  opencode/  generic-agentsmd/
...
Runtime installs that live _outside_ the vault (e.g. Codex's `~/.codex/skills`) are written only with explicit consent during bootstrap and recorded in the lockfile with `location: runtime` so they can be audited and removed (§8.1).
...
Shipping a new profile must never require code. Initial roster: `phd-student`, `researcher-developer` (the canonical example), `fitness-focused`, `writer`, `minimal`.
...
Bootstrap executes the documented path for the user's chosen runtime, **pinned** to the commit recorded in `sources.obsidian-skills.pin`. These paths are facts about the world, not about this project — re-verify at build time (P9).
...
|---|---|---|
|`vault-curator`|core|conventions enforcement, link hygiene, frontmatter validation, periodic vault health report|
...
|`codex`|skills copied to `~/.codex/skills` (consent required; `location: runtime` in lock), generated `AGENTS.md` in vault|
|`opencode`|repo-shaped skill layout under `~/.opencode/skills/` (consent required), generated `AGENTS.md`|
|`generic-agentsmd`|`AGENTS.md` in vault embedding conventions, roster, and skill references|

Runtime conventions churn quickly; adapters exist precisely to quarantine that churn. Pre-1.0, only Claude Code carries a first-class promise — the rest are skills-level support (D9).
...
`location` is `vault` or `runtime` (files installed outside the vault, e.g. Codex skill copies), so external writes are auditable and reversible.
...
|`doctor`|validate vault state against intent|read-only; network checks opt-in|
...
Config schema validity · module dependency closure · lockfile↔disk consistency (missing, modified, orphaned managed files) · template presence for enabled modules · frontmatter conformance of framework-created notes · runtime install presence for declared runtimes · pin reachability (network — opt-in flag only). `doctor` never modifies anything; it emits a report with suggested commands.
...
|Obsidian Bases syntax evolution|med|med|`.base` assets validated by `doctor`; track upstream syntax docs; golden examples pinned|
...
1. Confirm or veto D1–D9; resolve D10 (the name).
