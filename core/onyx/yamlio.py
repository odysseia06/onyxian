...
def load_yaml(path: Path, *, what: str) -> object:
    ...
    try:
        ...
        raise OnyxError(f"{what} not found: {path}") from None
    except OSError as exc:
        raise OnyxError(f"cannot read {what} {path}: {exc}") from None
    try:
        data = yaml.safe_load(text)
    except yaml.YAMLError as exc:
        raise OnyxError(f"{what} {path} is not valid YAML: {exc}") from None
    return {} if data is None else data


def require_mapping(data: object, *, what: str) -> dict:
    if not isinstance(data, dict):
        raise OnyxError(f"{what} must be a YAML mapping, got {type(data).__name__}")
    return data
