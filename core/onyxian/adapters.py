...
from .compat import REDIRECTOR_LINUX, REDIRECTOR_MACOS, REDIRECTOR_WINDOWS
...
_OPERATING_PREAMBLE = [
    ...
    f"(on Windows, `{REDIRECTOR_WINDOWS}`).",
...
def resolve_agents(
    ...
    """Every enabled module's agents, rendered and scope-validated exactly once.

    The single resolution site (#68): every artifact that speaks about agents —
    the rendered subagent files, AGENTS.md, the ``.claude/onyxian.md`` digest,
    ``Onyxian Assistant.md``, and ``onyxian-scopes.json`` — consumes this list,
    so their prose and scopes can never drift apart.
    """
    ...
    for manifest in manifests:
        ...
        for agent in manifest.agents:
            ...
            agents.append(ResolvedAgent(agent, ctx, enabled_modules, origin=origin))
    return agents


def render_agent_markdown(resolved: ResolvedAgent, *, scope_hooks: bool = False) -> str:
...
def claude_orientation_intents(
    ...
    roster: list[str] = []
    for resolved in agents:
        line = f"- **{resolved.name}** — {resolved.description}"
        if resolved.triggers:
            line += f' Say e.g. "{resolved.triggers[0]}".'
        roster.append(line)
    skill_ids = [skill.id for manifest in manifests for skill in manifest.skills]
    ...
    digest = [
        ...
        f"(Windows `{REDIRECTOR_WINDOWS}`, "
        f"macOS `{REDIRECTOR_MACOS}`, Linux `{REDIRECTOR_LINUX}`).",
...
def assistant_guide_intent(
    ...
    agent files and `onyxian.md`. Managed, core-attributed, claude-code-gated; no
    dates so golden trees stay byte-exact. Rendered even for a core-only vault
    (no agents).
    """
    if "claude-code" not in config.runtimes:
        return None
...
def claude_scopes_intent(
    ...
    Built from the shared resolved-agent list so there is no second scope resolver —
    the JSON and the agent's own "You may write only within" prose come from the same
    ...
    if not config.scope_hooks or "claude-code" not in config.runtimes:
        return None
    scopes = {resolved.name: {"write": resolved.write} for resolved in agents}
    if not scopes:
        return None
...
def claude_code_intents(
    ...
    """
    versions = {m.name: m.version for m in manifests}
    intents: list[FileIntent] = []
    seen_agent_paths: set[str] = set()
    for resolved in agents if "claude-code" in config.runtimes else ():
        path = f"{CLAUDE_AGENTS_PREFIX}/{resolved.name}.md"
        if path in seen_agent_paths:
            raise ResolveError(f"agent name collision at {path}")
        seen_agent_paths.add(path)
        split_portable(path, origin=resolved.origin)
        content = encode_text(render_agent_markdown(resolved, scope_hooks=config.scope_hooks))
        intents.append(
            FileIntent(
                path=path,
                content=content,
                ...
                module_version=versions[resolved.module],
            )
        )
